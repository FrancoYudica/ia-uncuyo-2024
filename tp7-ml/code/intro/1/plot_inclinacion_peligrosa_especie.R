...
# Agrupar y contar las inclinaciones peligrosas por especie
data_summary <- dataset_filtered %>%
  group_by(especie, inclinacion_peligrosa) %>%
  summarise(count = n(), .groups = "drop")

# Crear un gráfico de barras
plot <- ggplot(data_summary, aes(x = especie, y = count, fill = inclinacion_peligrosa)) +
  geom_bar(stat = "identity", position = "dodge") +  # Usar "dodge" para mostrar barras lado a lado
  labs(title = "Inclinaciones Peligrosas por Especie", 
       x = "Especie", 
...
ggsave("inclinacion_peligrosa_especie.jpg", plot = plot, width = 10, height = 6)
